...
public class PlayerData {
	...
	public List<String> getUnlockedAbilities() {
		...
		for (String ability : path.getAbilities()) {
			if (experience.containsKey(path) && path.getAbilityLevels().get(ability) <= getLevel(path)) {
	...
	public void addExp(PlayerPath path, double experience) {
		...
		if (currentLevel != newLevel) {
			...
			if (newLevel >= path.getMaxLevel()) {
				setExperience(path, plugin.methods.getExperience(path.getMaxLevel()));
	...
	public double expRequiredToLevel(PlayerPath path) {
		return this.experience.containsKey(path)
				? plugin.methods.getExperience(getLevel(path)) - getExperience(path) : -1;
	}
	...
	/*
	 * ExpRequiredToLevelUp * How much experience they need to level up - MAYBE
	 * PUT IN BLMETHODS
	 * 
